...
/**
 * THROWAWAY /lab avatar in the same flat, bold-outline CARTOON language as the
 * Go-Crux boulder mascots — chunky shapes, warm plum outline, dot-and-dash
 * eyes, rosy blush, soft warm backdrop. Reads the live AvatarConfig so it's a
 * true drop-in comparison against the enamel-pin renderer.
 */
const OUTLINE = '#2A2140';
const SW = 3.5;

/** Bucket the 18 hair styles into a few cartoon silhouettes. */
...
const BACKDROP_HEX: Record<string, string> = {
  ink: '#FDE9C8',   // warm cream (never dark — cartoon reads happy)
  teal: '#CFF3EA',
  gold: '#FFE6A8',
  coral: '#FFD9D2',
  sky: '#D8E6FF',
...
export function CartoonAvatar({
  ...
  // ── eyes (dot-and-dash, mascot language) ──
  ...
  const eyes = (() => {
    switch (config.eyes) {
      ...
      default: // sharp — two dots + light brows
        return (
          <g>
            {dot(42)}{dot(58)}
  ...
  return (
    <svg width={size} height={size} viewBox="0 0 100 100" fill="none" aria-label="cartoon avatar">
      {/* backdrop */}
      {ring ? (
        <>
      ...
      {/* front hair fringe hint for bob/long */}
      {beanie && accessory}
    </svg>
  );
}
