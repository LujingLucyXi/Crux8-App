 ...
 * a confetti burst. Fires on earned events (send, check-in, recap, level up).
 */
export function Celebration({ show, onDone, sound = true }: Props) {
  useEffect(() => {
    if (!show) return;
      ...
      {show && (
        ...
        >
          <motion.div
            ...
            style={{ background: `radial-gradient(circle at 50% 42%, ${show.colors[0]}33, rgba(6,4,20,0.62))` }}
          />
          ...
          >
            <motion.div
              ...
              style={{ background: `conic-gradient(from 210deg, ${show.colors.join(', ')}, ${show.colors[0]})` }}
